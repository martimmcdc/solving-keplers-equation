"""
This script is an implementation of Murison's method for solving Kepler's equation.
The key part of this method is the initial guess.
...
def murison_solver(e,M,epsilon=1e-9,iter_counter=False):
    Mnorm = M%(2*np.pi)
...
if __name__=="__main__":
    ...
    # Time the function
    init = time.time()
    E_out,iters = murison_solver(e,M_input,iter_counter=True)
    ...
    for power in range(6,16,3):
        for i in range(len(e)):
            vals = murison_solver(e[i],M,epsilon=10**(-power))
